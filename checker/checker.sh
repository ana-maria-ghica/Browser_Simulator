...
function valgrindTest() {   
    ...
    valgrind --leak-check=full --track-origins=yes -q --log-file=rezultat_valgrind.txt ./main < file.in > /dev/null
...
function testInput() {
    ...
    cat "$file" > file.in
    dos2unix -q file.in
    timeout 10s ./main < file.in > file.out
    ...
    if ! diff -B -Z --strip-trailing-cr "$ref_file" file.out > /dev/null 2>&1; then
